...
Loop 16 extracts `useExportHandlers` hook from ImportExportPage: 5 inline export `useCallback` blocks (`handleCopyLink`, `handleExportJSON`, `handleExportCSV`, `handleExportMarkdown`, `handleExport`) + `downloadFile` util moved behind a stable Interface. ImportExportPage 401→253 LOC. 5 new Interface-level tests at `useExportHandlers.test.ts`. Suite: 27 suites, 184 tests, all green. simplicity 7.5→8.0.

## Scorecard (1-10)
- Architecture quality: 7.5 | UP | `apps/web/src/hooks/useExportHandlers.ts:1-182` — 5-selector export dep-cluster now behind Interface; ImportExportPage reduced to orchestration-only role. Package DAG enforced. 9-anchor still not met: domain model anemic, store implicit global, page-level tests absent.
...
- Framework / platform best practices: 7.5 | UP | `apps/web/src/hooks/` — 11 focused hooks (up from 10); RTK patterns correct; `useId()` for stable IDs; ImportExportPage now at 253 LOC with 3 hook delegations (`useImportHandlers`, `useExportHandlers`, `useExport`). No undocumented carve-outs.
...
- Code simplicity and clarity: 8.0 | UP | `apps/web/src/hooks/useExportHandlers.ts:1-182` — 5 export handlers + `downloadFile` extracted from ImportExportPage; page 401→253 LOC. 5 `useCallback` blocks + 3 `useAppSelector` calls removed from page. `useExportHandlers` returns `{onCopyLink, onExportJSON, onExportCSV, onExportMarkdown, onExport}` — clean destructured Interface.
- Test strategy and regression resistance: 8.0 | SAME | Suite: 27 suites, 184 tests. `useExportHandlers.test.ts` — 5 tests: JSON download filename, CSV filename, Markdown filename, onExport routes json, onExport routes png. Authority Map cross-check: `useExportHandlers` Interface directly tested. Remaining ceiling: page-level surfaces still untested — 9-anchor not met.
- Overall implementation credibility: 8.0 | SAME | `useExportHandlers` passes deletion test: 5-selector complexity (URL generation, JSON structure, CSV/Markdown formatting, downloadFile) vanishes from page. Replace-don't-layer satisfied: no prior tests for these handlers; 5 new tests at new Interface. Implementation reviewer approved.
...
- Structurally necessary: `useExportHandlers` extraction — `handleCopyLink`, `handleExportJSON`, `handleExportCSV`, `handleExportMarkdown`, `handleExport` all share 5 identical selectors (`projectName`, `tierOrder`, `tierLabels`, `tierColors`, `tiers`). Extraction concentrates the dep cluster + all format logic behind a stable Interface. Deletion test passes: complexity vanishes from ImportExportPage.
- New seam justified: No new architectural Seam introduced. `useExportHandlers` is an in-process hook, not a protocol/port abstraction.
- Helpful simplification: ImportExportPage.tsx 401→253 LOC; 5 `useCallback` blocks removed; 3 `useAppSelector` calls removed from page; `downloadFile` helper moved to hook file; `ExportFormatter` import removed from page.
- Should NOT be done: Wrapping `downloadFile` in a testability seam — it's a 6-line DOM utility. Merging `useExportHandlers` with `useExport` — different concerns (text serialization vs image capture).
- Tests after fix: `apps/web/src/hooks/useExportHandlers.test.ts` — 5 tests at new Interface. No old tests to delete (Replace-don't-layer: no prior tests existed for these handlers).
...
Good app, place but not win. Loop 16 executes `useExportHandlers` extraction: 5-selector dep cluster + format serialization + downloadFile moves behind stable Interface; ImportExportPage 401→253 LOC. simplicity 7.5→8.0, architecture_quality 7.0→7.5, framework_idioms 7.0→7.5: structural proof is the hook + removed useCallback blocks + 5 new Interface tests. Two loops remain at cap 18. Priority 1 and 2 are residual acceptances — no further extraction candidates pass the deletion test at the current scope. Average score holding near 7.3. Remaining sub-9.5 scores reflect honest structural blockers (anemic domain model, implicit global store, no process-lifetime pattern) — not addressable in remaining budget.
