...
import { Tun, UNRANKED_TIER_ID } from "./constants";
import {
    churnFraction,
    quantileCuts,
    tierMapForCuts,
...
function slice(items: readonly Item[], range: readonly [number, number]): Item[] {
...
export function adjustedRefinedCuts(
    primaryCuts: readonly number[],
    quantCuts: readonly number[],
    tierCount: number,
    ordered: readonly Item[],
    metrics: Record<string, HeadToHeadMetrics>
): number[] {
...
export function pickPair(
    ...
    return pickRandomPair(pool as Item[], rng);
}
...
export function vote(
    ...
    if (winner.id === a.id) {
        ...
        // @ts-ignore - readonly bypass for mutation
        recA.wins += 1;
        // @ts-ignore
        recB.losses += 1;
    } else {
...
class WarmStartQueueBuilder {
    ...
    constructor(pool: readonly Item[], target: number) {
        this.target = target;
        for (const item of pool) {
    ...
    enqueueUnranked(unranked: readonly Item[], anchors: readonly Item[]): boolean {
    ...
    enqueueFallback(pool: readonly Item[]): void {
